...
- ai.test.ts › Elite bets top set into a checked-through range on a dry board: default 5,000 ms budget → 20,000 ms (80 production-depth Elite decisions ≈ 4–6 s), reason: range-based equity and called-equity passes; assertion unchanged; test renamed from "Elite bets a strong hand into a weak checked range far more than a capped-and-strong one" because the body makes no comparison
- coordinator.test.ts › keeps a six-seat cross-street AI batch chronological and returns control only to the acting human: seededRandom(1) → seededRandom(2), reason: AI decisions changed in c4289145 (pre-branch commit); the fixture no longer reached the flop; assertions unchanged
- coordinator.test.ts › R3 › lets a connected positive-stack sitting-out human…: seed 991 → 993, reason: under the range model both host and guest busted in hand one on the old seed; assertions unchanged
