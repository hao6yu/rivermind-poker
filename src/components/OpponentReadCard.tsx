...
export function OpponentReadCard({ memory, onReset, privacyNote = false }: OpponentReadCardProps) {
  ...
  return (
    <View accessibilityLabel={`Opponent read. ${read.title}. ${read.detail}`} accessible style={styles.card}>
      <View style={styles.header}>
        ...
        <View style={styles.copy}>
          <Text style={styles.eyebrow}>
            What opponents see · {read.confidenceLabel} · {memory.handsObserved} hand{memory.handsObserved === 1 ? '' : 's'}
          </Text>
      ...
      <View style={styles.metrics}>
        <ReadMetric label="Played preflop" value={formatRate(memory.voluntaryPreflopHands, memory.preflopOpportunities)} />
        <ReadMetric label="Raised preflop" value={formatRate(memory.preflopRaises, memory.preflopOpportunities)} />
        <ReadMetric label="Folded to bet" value={formatRate(memory.foldsFacingBet, memory.facedBetOpportunities)} />
      </View>
      {privacyNote || (onReset && memory.handsObserved > 0) ? (
        <View style={styles.footer}>
          {privacyNote ? (
            <Text style={styles.privacy}>Only your visible actions and seat position are remembered on this device—never cards or the undealt deck.</Text>
          ) : null}
          {onReset && memory.handsObserved > 0 ? (
            <Pressable accessibilityRole="button" onPress={onReset} style={styles.resetButton}>
              <Text style={styles.resetText}>Reset read</Text>
            </Pressable>
          ) : null}
        </View>
      ) : null}
...
function createStyles(palette: ThemePalette) {
  return StyleSheet.create({
    ...
    metricLabel: { color: palette.muted, fontSize: 7.5 },
    ...
    resetText: { color: palette.primary, fontSize: 9, fontWeight: '800' },
  });
}
