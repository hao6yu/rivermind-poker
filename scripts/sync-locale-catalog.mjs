...
if (localeIndex === -1 || !args[localeIndex + 1]) throw new Error('Usage: sync-locale-catalog.mjs --locale <es-419|pt-BR|ja>');
const locale = args[localeIndex + 1];
if (locale !== 'es-419' && locale !== 'pt-BR' && locale !== 'ja') throw new Error(`Unsupported locale ${locale}`);

/**
 * One configuration entry per supported locale (Phase 19.5: the per-locale
 * ternary chain became a map instead of growing). `phaseSuffix` names the
 * generated phase exports (phase7<phaseSuffix>Messages), `allowSuffix` the
 * registry key suffix used by the language self-name allowlist.
 */
const LOCALE_CONFIG = {
  'es-419': {
    dir: 'es419',
    ...
    phaseSuffix: 'Spanish',
    selfNameKeySuffix: 'es419',
    learningName: 'spanishLearningContent',
    scenarioTemplatesName: 'spanishScenarioTemplates',
    scenarioVocabName: 'spanishScenarioVocab',
    draftLine: '// DRAFT: awaiting qualified native es-419 poker-language review.',
  },
  'pt-BR': {
    dir: 'ptbr',
    baseName: 'basePortugueseMessages',
    phaseSuffix: 'Portuguese',
    selfNameKeySuffix: 'ptBr',
    learningName: 'portugueseLearningContent',
    scenarioTemplatesName: 'portugueseScenarioTemplates',
    scenarioVocabName: 'portugueseScenarioVocab',
    draftLine: '// DRAFT: awaiting qualified native pt-BR poker-language review.',
  },
  ja: {
    dir: 'ja',
    baseName: 'baseJapaneseMessages',
    phaseSuffix: 'Japanese',
    selfNameKeySuffix: 'ja',
    learningName: 'japaneseLearningContent',
    scenarioTemplatesName: 'japaneseScenarioTemplates',
    scenarioVocabName: 'japaneseScenarioVocab',
    // Phase 19.5: native Japanese review is pending (owner gate) and the
    // generated modules must keep saying so until it is recorded.
    draftLine: '// DRAFT: awaiting qualified native ja poker-language review (Phase 19.5; docs/LOCALIZATION_JA_STYLE_GUIDE.md §13).',
  },
};
...
const SHARED_VALUE_ALLOWLIST = new Set([
  ...
  'language.zhHans',
  'language.zhHant',
  `language.${config.selfNameKeySuffix}`,
...
const phaseParts = phaseNames.map((phase) => {
  const exportName = `${phase}${config.phaseSuffix}Messages`;
  return {
    exportName,
...
const scenarioErrors = [];
{
  ...
  const vocabName = config.scenarioVocabName;
  const templatesName = config.scenarioTemplatesName;
