-- Keep one active recipient per canonical push address across anonymous reinstalls.
-- Fingerprints retain delivery suppression without copying gameplay/account data.
-- This rollout is still disabled and has no production delivery history.
create index notification_deliveries_token_time on public.notification_deliveries(token_fingerprint,created_at desc) where token_fingerprint is not null;
create unique index notification_deliveries_token_content_once on public.notification_deliveries(token_fingerprint,content_id) where token_fingerprint is not null;
create unique index notification_deliveries_token_release_once on public.notification_deliveries(token_fingerprint,release_version) where token_fingerprint is not null and release_version is not null;
...
begin
  ...
  delete from public.notification_recipients where expo_token=canonical_token and user_id<>p_user_id;
  ...
  values (p_user_id, (p_device->>'installationId')::uuid, canonical_token, p_device->>'platform', p_device->>'locale',
...
begin
  for r in
    ...
    where rollout.enabled and (rollout.production_enabled or n.user_id = any(rollout.test_user_ids))
      ...
      and not exists(select 1 from public.notification_deliveries d where (d.user_id=n.user_id or d.token_fingerprint=md5(n.expo_token)) and d.created_at>now()-interval '72 hours')
      and (select count(*) from public.notification_deliveries d where (d.user_id=n.user_id or d.token_fingerprint=md5(n.expo_token)) and d.created_at>now()-interval '7 days')<2
      and (select count(*) from public.notification_deliveries d where (d.user_id=n.user_id or d.token_fingerprint=md5(n.expo_token)) and d.created_at>=n.last_active_at)<2
      and exists(select 1 from public.notification_content m where m.enabled and m.starts_at<=now()
        ...
        and not exists(select 1 from public.notification_deliveries d where (d.user_id=n.user_id or d.token_fingerprint=md5(n.expo_token)) and (d.content_id=m.id or d.release_version=m.release_version)))
  ...
  loop
    ...
    if exists (select 1 from public.notification_deliveries d where (d.user_id = r.user_id or d.token_fingerprint=md5(r.expo_token)) and d.created_at > now() - interval '72 hours')
      or (select count(*) from public.notification_deliveries d where (d.user_id = r.user_id or d.token_fingerprint=md5(r.expo_token)) and d.created_at > now() - interval '7 days') >= 2
      or (select count(*) from public.notification_deliveries d where (d.user_id = r.user_id or d.token_fingerprint=md5(r.expo_token)) and d.created_at >= r.last_active_at) >= 2 then
      continue;
    end if;
    select m.kind into last_kind from public.notification_deliveries d join public.notification_content m on m.id = d.content_id
      where (d.user_id = r.user_id or d.token_fingerprint=md5(r.expo_token)) order by d.created_at desc limit 1;
    ...
    where m.enabled and m.starts_at <= now() and (m.expires_at is null or m.expires_at > now())
      ...
      and not exists(select 1 from public.notification_deliveries d where (d.user_id = r.user_id or d.token_fingerprint=md5(r.expo_token)) and (d.content_id = m.id or d.release_version=m.release_version))
    ...
    if found then
      ...
      values(r.user_id,c.id,r.installation_id,c.release_version,md5(r.expo_token)) returning id into delivery_id;
...
begin
  ...
  if r.user_id is null or not r.permission_granted or r.installation_id <> d.installation_id
    ...
    or d.token_fingerprint is distinct from md5(r.expo_token)
