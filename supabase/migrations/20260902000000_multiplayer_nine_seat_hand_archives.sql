-- Slice 3.11 Qwen follow-up (Q1): nine-seat human tables could never settle.
--
-- Defect: `multiplayer_commit_transition_v2` hard-capped `p_hand_archives` at
-- 6 entries, but a nine-seat room deals up to NINE human viewer archives per
-- settled hand. Every seventh human settlement raised 22023
-- 'Invalid multiplayer transition collections.', the worker answered 503
-- room_unavailable, and the entire room became unplayable past hand 1.
--
-- Fix: the bound becomes the approved table maximum — nine archives, never
-- more than the human seats of the submitted canonical state — while keeping
-- the collection fail-closed:
--   * > 9 archives (or more than the submitted canonical state's human seats)
--     is refused;
--   * duplicate viewer identities inside one transition are refused (the old
--     ON CONFLICT DO NOTHING silently swallowed them);
--   * the existing per-archive authorization (viewer owns a human seat of the
--     submitted state), redaction validation, membership revocation, and
--     version/session guards are preserved exactly.
--
-- Deployment ordering: apply this migration BEFORE the follow-up worker build
-- (Q2 archives are a subset of the previous producer's, so the old worker
-- stays compatible with the new validation).
...
begin
  if jsonb_typeof(p_public_actions) <> 'array'
    ...
    or jsonb_array_length(p_hand_archives) > 9 then
    raise exception using errcode = '22023', message = 'Invalid multiplayer transition collections.';
  end if;

  -- Q1: archives are bounded by the actual human roster of the SUBMITTED
  -- canonical state (which the seat loop below re-validates), never by the
  -- old six-seat assumption. A canonical state without a seats array cannot
  -- own archives at all.
  human_seat_count := case
    when jsonb_typeof(p_canonical_state->'seats') = 'array' then (
      select count(*)
      from jsonb_array_elements(p_canonical_state->'seats') as seat_row(value)
      where seat_row.value->>'kind' = 'human'
    )::integer
    else 0
  end;
  if jsonb_array_length(p_hand_archives) > human_seat_count then
    raise exception using errcode = '22023', message = 'Invalid multiplayer transition collections.';
  end if;
  if exists (
    select 1
    from (
      select archive_row.value->>'userId' as viewer_user_id
      from jsonb_array_elements(p_hand_archives) as archive_row(value)
      group by 1
      having count(*) > 1
    ) duplicate_archive
  ) then
